...
class DocumentHigh {
    ...
    public function propertyDocumentPreview ( $propertyId = '', $userId = '', $userIP = '', $documentType = '') {
        ...
        if ( !empty( $property ) && $property != null ) {
            ...
            $highSecurityFolder = $propertyDocBlobName.'/'.$property->URLSlug.'/High/';
            $highSecurityFolder = Storage::disk('azure')->allFiles($highSecurityFolder);

            $documentVaultTreeHigh = [];
            if( !empty($highSecurityFolder)) {
                foreach ($highSecurityFolder as $key => $value ) {
                    $highSecurityFolder[$key] = str_replace($docSlug, '', $value);
                }
                $documentVaultTreeHigh = self::createTreeArray($highSecurityFolder);
            }

            $highFolderEmptyFlag = false;

            if( empty($highSecurityFolder)) {
                $highFolderEmptyFlag = true;    
            }
            ...
            if( !empty($directories) || !empty($highSecurityFolder) ) {
                ...
                if( empty($documentVaultTree) ) {
                    $dddocflag = true;
                }
                $documentPreviewHtml = self::createHTML($documentVaultTree, $documentVaultTreeHigh, $highFolderEmptyFlag);
     ...
     * @return string
     */
    public function createHTML( $data = array(), $highData = array(), $highFolderEmptyFlag = false) {
        $documentPreviewHighHtml = "";
        if ( !empty($highData) ) {
            $documentPreviewHighHtml = self::createHighHTML( $highData, $highFolderEmptyFlag);
        } else {
            $documentPreviewHighHtml = self::createHighHTML( $highData, $highFolderEmptyFlag);
        }
        ...
        $html .= '<ul class="oepl_Folder_FileLists master_ul_document_vault">';
            ...
            $html .= '<li>';
                ...
                foreach ($data as $key => $value) {
                    ...
                    if ( !$highFolderEmptyFlag ) {
                        $html .= '<li class="high_folder_tree_vew">';
                            $html .= $documentPreviewHighHtml;
                        $html .= '</li>';
                    }
    ...
    public function createHighHTML( $data = array(), $highFolderEmptyFlag) {
        if( !empty($data)) {
            $html .= '<li>';
                $html = '<div class="document_heading_section">';
                    $html .= '<div class="title_name">High Security</div>';
                    $html .= '<div class="title_added_date">Date Added</div>';
                $html .= '</div>';
                foreach ($data as $key => $value) {
                    $html .= '<ul class="oepl_Folder_FileLists oepl_sub_folder main_folder">';
                    $html .= self::recursiveHighLoop($value ,$html);
                    $html .= '</ul>';
                }
            $html .= '</li>';
            return $html;
    ...
    public function recursiveLoop($datas, $html = '', $mainFolder = 'yes'){
        ...
        if(!empty($datas) ){
            ...
            foreach ($datas as $k => $v) {
                if(!empty($v)){
                    ...
                    if( !empty($datumDirectoryFile)) {
                        ...
                        $checkbox_html .= '<input class="oepl_checkbox selectedfilename" type="checkbox" name="filename[due_diligence]['.$datumDirectoryFile['fileid'].'][]" data-directoryfileid="" value="'.$k.'" data-val="" data-filesize="'.$FileSize.'"/><span class="k-checkbox-label checkbox-span">';
            ...
            $html = self::docuementNotHighFoundHtml();
        }
        return $html;
    }

    public function recursiveHighLoop($datas, $html = '', $mainFolder = 'yes'){
        if(!empty($datas)){
            $html = '';
            foreach ($datas as $k => $v) {
                if(!empty($v)){
                    ...
                    $html .= '<li class="'.$folderCLass.'">';
                        ...
                        $html .= '<ul class="oepl_Folder_FileLists oepl_sub_folder oepl_folder_closed">';
                            $html .= self::recursiveHighLoop($v ,$html, 'no');
                        $html .= '</ul>';

                    $html .= '</li>';
                }else{
                    $icon = "";
                    $datumDirectoryFile = self::getHighFileId($k, $this->propertySlug);
                    if( !empty($datumDirectoryFile)) {
                        //$mimeType = self::getMIMEType($k);
                        $mimeType = $datumDirectoryFile['mimetype'];
                        $ModifiedDate = $datumDirectoryFile['ModifiedDate'];
                        ...
                        $checkbox_html .= '<input class="oepl_checkbox selectedfilename" type="checkbox" name="filename[high]['.$datumDirectoryFile['fileid'].'][]" data-directoryfileid="" value="'.$k.'" data-val="" data-filesize="'.$FileSize.'"/><span class="k-checkbox-label checkbox-span">';
                        $checkbox_html .= '</span></label>';
                        $html .= $checkbox_html;
                        $html .= '<span class="">';
                        $fileName = substr($k, strrpos($k, '/') + 1);
    ...
    public function getHighFileId($fileUrl = "", $propertySlug) {
        if($fileUrl != "" ) {
            $dt = explode('/', $fileUrl);
            $lastDirName = $dt[sizeof($dt)-2];
            $fileName = $dt[sizeof($dt)-1];
            ...
            $fileQuery->where('DatumDirectory.DirectoryName', $lastDirName);
            $fileQuery->where('DatumDirectoryFile.IsDeleted', 0);
            $fileQuery->where('DatumDirectoryFile.FileType', 'High');
            $fileQuery->select("DatumDirectoryFile.*");
            $directoryFiles = $fileQuery->first();
    ...
    public function docuementNotFoundHtml($documentType = "") {
        $html = '';
        $html = '<div class="document_heading_section">';
            $html .= '<div class="title_name">Due Diligence</div>';
            $html .= '<div class="title_added_date">Date Added</div>';
        $html .= '</div>';
     ...
     * @return string
     */
    public function docuementNotHighFoundHtml($documentType = "") {
        $html = '';
        $html = '<div class="document_heading_section">';
            $html .= '<div class="title_name">High Security</div>';
            $html .= '<div class="title_added_date">Date Added</div>';
